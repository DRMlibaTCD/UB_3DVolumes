...
Built by **Mlibatisi Dlamini** · B.Sc. Geomatics, University of Botswana
...
- **Plan and 3D perspective views** of the triangulated, elevation-coloured surface, so you can sanity-check that it actually looks like the site before trusting the numbers.
- **Cut / fill / net volume**, plan area, and surface area, computed live against a reference elevation you set (or pick from the data's min/max).
- **Downloadable report** (`.txt`) recording every input, setting, and result for a lab writeup or client email.
...
Where the numbers matter for grading or a client, verify against licensed software. Where they don't, this should save you the wait for a lab seat.
