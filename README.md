...
A free, browser-based surface volume calculator for earthworks and site surveys — no license, no install, no lab booking. Built with Geomatics coursework in mind, but useful for any engineering or science student (or anyone else) who needs quick cut/fill numbers without a licensed GIS/CAD seat.
...
- **CSV or text file upload** with automatic X/Y/Z column detection and auto-detected delimiter (comma, tab, semicolon, pipe, or space), or a built-in sample dataset to try it instantly.
...
- **Explicit confirmation before triangulating** — once your columns and reference elevation are set, TerraVol asks "ready to see results?" rather than recomputing and displaying automatically; changing any setting afterward re-asks.
- **Plan and 3D perspective views** of the triangulated surface. The 3D view can be dragged to rotate (orbit azimuth and tilt) or panned, toggle between the two; the plan view pans. Both have a "Open larger" fullscreen view and a "Download image" (PNG) export.
...
- **Dark / light mode** toggle, shown as a labelled button in the header (not just an icon).
- **Cut / fill / net volume**, plan area, surface area, coordinate extent, and the lowest/highest surveyed points — all computed against a reference elevation you set (or pick from the data's min/max).
...
- **Built-in onboarding** — a short "how to use this" panel on first load, reopenable any time from the header.
...
Open `index.html` in a browser — that's the whole app, no server or build step required. Upload a CSV or text file with X, Y, Z columns (and optionally a code and/or breakline column), set a reference elevation, confirm when asked, and read the results.
